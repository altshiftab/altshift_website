import "@altshiftab/styles/common.css";
import "../styles/common_www.css";
import "../styles/index.css";

import "@altshiftab/web_components/button";

import "./custom_elements/altshift_header_www.ts"
import "./custom_elements/altshift_footer_www.ts"
import "./custom_elements/services_table.ts";
import "./custom_elements/customers_table.ts";
import "./custom_elements/customer_quote.ts";
import "./custom_elements/contact_us.ts";

document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll("#splash > altshift-box").forEach(box => box.classList.toggle("active"));
});
